...
from .diff import build_diff_markdown, build_diff_payload
from .engine import (
    SEVERITY_ORDER,
...
def _run_diff(args: argparse.Namespace) -> int:
    ...
    if args.current:
        ...
        current_findings = list(current_payload.get("findings", []))
        repo = str(current_payload.get("repo", Path(args.current).stem))
    else:
        target = Path(args.target).resolve()
        current_payload = build_scan_payload(target, min_confidence=args.min_confidence)
        current_findings = list(current_payload["findings"])
        repo = str(current_payload["repo"])

    diff_payload = build_diff_payload(repo, current_findings, baseline_payload)

    if args.markdown:
        _write_or_print(build_diff_markdown(diff_payload), args.output)
    else:
        _write_or_print(json.dumps(diff_payload, indent=2), args.output)

    if args.fail_on_new and diff_payload["summary"]["new"] > 0:
        return 1
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    diff = sub.add_parser(
        "diff",
        help="Compare a scan against a baseline and surface only newly introduced findings.",
    )
    diff.add_argument("target", nargs="?", default=".", help="Repository path to rescan.")
    diff.add_argument("--baseline", required=True, help="Baseline findings payload (from an earlier scan).")
    diff.add_argument("--current", help="Use a saved scan payload instead of rescanning target.")
    diff.add_argument("--output", help="Optional file to write the drift report to.")
    diff.add_argument("--min-confidence", type=float, default=0.0, help="Drop findings below this confidence when rescanning.")
    diff.add_argument("--markdown", action="store_true", help="Render a Markdown report instead of JSON.")
    diff.add_argument("--fail-on-new", action="store_true", help="Exit non-zero if any new finding is introduced.")
